...
internal abstract class Bt22BlockedCardScript : ICardScript
{
    protected Bt22BlockedCardScript(
        string cardId,
...
internal static class Bt22ScriptSupport
{
    ...
    public static PlayerId RequireSourceCardOwner(GameState state, CardInstanceId? sourceCard)
    {
        ...
        {
            throw new DomainException("BT22 card script requires a source card owner.");
        }
    ...
    {
        var card = sourceCard
            ?? throw new DomainException("BT22 security play-self requires a source card.");

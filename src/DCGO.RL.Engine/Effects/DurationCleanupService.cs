...
public sealed class DurationCleanupService
{
    public DurationCleanupResult CleanupTurnEnd(GameState state, PlayerId endingTurnPlayer)
    {
        ...
        return new DurationCleanupResult(DurationCleanupEvent.TurnEnd, removed);
    }
    ...
    public DurationCleanupResult CleanupSecurityCheckEnd(GameState state)
    {
        ...
        return new DurationCleanupResult(DurationCleanupEvent.SecurityCheckEnd, removed);
    }
    ...
    public DurationCleanupResult CleanupOwnerActivePhaseEnd(GameState state, PlayerId activePlayer)
    {
        ...
        return new DurationCleanupResult(DurationCleanupEvent.OwnerActivePhaseEnd, removed);
    }
    ...
    public DurationCleanupResult CleanupStaleTargets(GameState state)
    {
        ...
        var removed = RemoveWhere(state, modifier => IsStaleTarget(state, modifier));
        return new DurationCleanupResult(DurationCleanupEvent.StaleTarget, removed);
    }

    private static IReadOnlyList<string> RemoveWhere(GameState state, Func<TemporaryModifier, bool> shouldRemove)
    {

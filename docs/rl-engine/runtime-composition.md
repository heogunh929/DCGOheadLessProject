...
현재 session resume은 hand option `OptionSkill`, chained option selection, optional yes/no + explicit target selection, normal play `OnPlay`, normal digivolve `WhenDigivolving`, attack lifecycle의 `OnAllyAttack`, `OnCounterTiming`, counter 선택, blocker selection, `OnBlockAnyone`, `OnAttackTargetChanged`, `OnEndAttack`, attack security check 중 `SecuritySkill`/Activate Main selection, rules timing, `OnStartTurn`, `OnStartMainPhase`, `PassAction`의 `OnEndTurn`/`OnStartTurn` pending selection을 처리한다. play/digivolve/attack/security/rule/phase runner는 pending 시 continuation을 반환하고, session resume은 selection 적용 후 남은 pipeline queue, attack tail, security-check loop, action tail을 이어서 처리한다.
...
Queue 52A는 hand option, chained option, `OnPlay`, `WhenDigivolving`, `OnAllyAttack`, `OnEndAttack`, attack security check 중 `SecuritySkill`/Activate Main selection, rules timing, `OnStartMainPhase`, `PassAction`의 `OnEndTurn`/`OnStartTurn` pending selection을 `EngineSession` boundary로 재개하는 기반을 추가했다. Queue 55A는 이 boundary에 counter 선택, blocker selection, `OnBlockAnyone`, `OnAttackTargetChanged` attack tail을 추가하고 attack runtime state를 `GameState.RuntimeRules.Attack`으로 이동했다. `OnEndBlockDesignation`은 원본 호출 근거가 없어 생성하지 않는다. 다음 범위는 아직 남아 있다.
...
## Queue 66J static requirement service graph

66J부터 source/condition-aware static digivolution/link requirement는 `StaticRequirementService`가 담당한다.

- production `BattleEngineServices.Create(...)`는 하나의 `StaticRequirementService`를 생성한다.
- `DigivolveService`, `ComplexMechanicService`, `LegalActionGenerator`는 같은 `StaticRequirementService` instance를 공유해야 한다.
- `BattleEngineServices.ValidationReport`는 해당 shared graph mismatch를 실패로 보고한다.
- `RandomLegalActionRunner`는 services graph가 제공하는 `LegalActionGenerator`를 사용한다. 별도 generator를 주입하지 않는 한 static requirement를 모르는 fallback generator를 사용하지 않는다.
- requirement descriptor는 GameState에서 파생 평가되며 service 자체는 rule-visible mutable state를 소유하지 않는다.

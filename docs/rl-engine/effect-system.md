...
- `DurationScope.UntilBattleEnd`는 permanent battle 또는 security battle 직후 `CleanupBattleEnd`에서 제거되고, `DurationScope.UntilAttackEnd`는 `AttackService`의 attack cleanup 시 `CleanupAttackEnd`에서 제거된다. 따라서 `OnEndAttack` 중에는 attack-end modifier가 남아 있고 battle-end modifier는 이미 사라진다.
- `BattleKeywordService.SecurityAttackCount`는 base 1에 static/temporary/continuous modifier를 합산한 뒤 0 미만으로 내려가지 않도록 clamp한다.
- `StaticEffectService.EffectiveCardColors`는 base `CardDefinition` 색상을 직접 수정하지 않고, static card color descriptor를 base/current layer 순서로 적용해 option color requirement와 digivolution color check에 쓰는 derived color를 계산한다.
...
- `IgnoreDigivolutionRequirement` permission은 66M에서 descriptor 기반 static evolution requirement fallback으로 연결했다.
- target permanent temporary keyword grant는 66N에서 `TemporaryModifierKind.Keyword`와 `Tier1PrimitiveService.AddTemporaryKeyword(...)`로 연결했다. full-card parity evidence는 66P에서 보수적 `NotRun` evidence로 생성했고, generated/runtime status mismatch는 66Q에서 legacy pilot divergence로 분리해 닫았다. `ICannotIgnoreDigivolutionConditionEffect` 대응 restriction layer는 66R에서 static requirement permission 경로에 연결했다. 66S에서는 player-wide temporary keyword grant를 `Tier1PrimitiveService.AddTemporaryPlayerKeyword(...)`와 metadata-gated battle-area Digimon target으로 연결했다. 66T에서는 duration-bound granted trigger source/timing flow를 `TemporaryGrantedEffect`와 `TemporaryGrantedEffectRegistry`로 연결했다. 66X에서는 static card color descriptor와 ignore color requirement descriptor를 option play/digivolution color gate에 연결했다. 66Y에서는 static card name/trait descriptor와 `CardMetadataSnapshot`을 static metadata criteria 경로에 연결했다. 66Z에서는 static card/permanent level descriptor를 normal digivolution 및 static evolution requirement level gate에 연결했다. 66AA에서는 static link requirement가 `StaticEffectService` effective metadata/level query를 공유하게 했다. 66AB에서는 static link cost modifier를 `CostResolver`의 link execution cost 경로에 연결했다. 66AC에서는 static cannot-play option restriction을 option play legal action과 execution gate에 연결했다. 66AD에서는 static cannot-put-field restriction을 hand permanent play와 effect-caused field entry gate에 연결했다. 66AE에서는 static cannot-move restriction을 return-to-hand permanent movement gate에 연결했다. `ContinuousOrStaticEffect`는 full-card source parity evidence가 아직 `NotRun`이므로 `PartiallyImplemented`로 유지한다.

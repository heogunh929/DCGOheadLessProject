...
| `TemporaryModifier`, `DurationCleanupService` | `CardEffectCommons` duration effect, `Player.EffectList(EffectTiming.None)` duration lists, `TurnStateMachine` turn-end cleanup lists, `AttackProcess` battle/security cleanup | runtime modifier를 상태에 저장하고 turn/battle/security/active cleanup에서 제거한다. 66G에서 player-target DP/SecurityAttack/SecurityDigimonDP stat modifier의 effective stat, clone/restore/hash, replay evidence를 추가했다. | 카드별 로직 없음 | 부분 적합 | 원본의 모든 until timing, cut-in/counter 전용 cleanup, next unsuspend류 scope와 non-stat static interface는 아직 전체 대응으로 보지 않는다. |
| `ContinuousEffectService`, `ContinuousEffectSourceCollector`, `EffectiveStatService`, `BattleKeywordService` | `ICardEffect` `EffectTiming.None`, background/continuous 효과, `Permanent`/`Player` stat query, `CardEffectFactory/KeyWordEffects/*SelfStaticEffect` | field top/inherited/linked/face-up security/hand/trash/executing source에서 continuous descriptor와 continuous keyword descriptor를 수집하고 effective DP/SecurityAttack 및 지원 keyword query를 파생 계산한다. player-level stat duration은 `TemporaryModifier` 경로와 합산된다. | 카드별 로직 없음. continuous card script가 stat 또는 keyword descriptor를 제공 | 부분 적합 | 66F에서 linked card와 face-up security source scope를 추가했고 66G에서 player-level runtime stat modifier evidence를 보강했다. 66H에서 hand/trash/executing source collection을 추가했고 66I에서 source/condition-aware static keyword descriptor path를 추가했다. static requirement, cost, restriction, immunity interface와 trait/name/text metadata, trigger형 keyword flow, full-card parity evidence는 아직 남아 있다. |

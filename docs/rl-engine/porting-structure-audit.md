...
3. ST2-07/ST3-07은 asset에서 shared `ST1_06`을 참조하지만 현재 `St2St3CardScriptCatalog`에는 `NoEffectCardScript`로 남아 있다.
...
1. `ST2-07`, `ST3-07`은 현재 registry가 `NoEffect`지만 원본 asset은 shared `ST1_06`을 참조한다. 이번 작업에서는 기능을 바꾸지 않고 marker/status/policy 문서에 위험을 기록했다.
